...
function supportRows(island: IslandSitemapHost): { host: MasterHost; path: string; priority: string }[] {
  return [...SUPPORT_PATHS, '/about', '/events', '/legal', '/journal', '/blog', '/locations', '/areas', '/contact', '/trust', '/services', '/help', '/fine-dining', '/staffing', '/corporate', '/gatherings', '/sitemap'].map((path) => ({
    host: island,
    path,
    priority: '0.6',
  }));
